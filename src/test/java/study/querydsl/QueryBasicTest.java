...
public class QueryBasicTest {
    ...
    public void search() {
        Member findMember = queryFactory
                .selectFrom(member).where(member.username.eq("member1").and(member.age.between(10, 30))).fetchOne();
        assertThat(findMember.getUsername()).isEqualTo("member1");
    ...
    */
    ...
    public void paging2() {
        QueryResults<Member> queryResults = queryFactory
                .selectFrom(member)
                .orderBy(member.username.desc()).offset(1).limit(2).fetchResults();
   ...
   */
    ...
    */
    ...
    public void concat() {
        // username_age
        // concat은 문자열 끼리 더할때 사용하기 떄문에 숫자형이나 Enum 타입을 사용할 때 stringValue()를 사용하여 문자열로 바꿔야 한다.
        List<String> result = queryFactory
    ...
    public void findDtoByJPQL() {
        List<MemberDto> result = em.createQuery("select new study.dto.MemberDto(m.username, m.age) from Member m", MemberDto.class).getResultList();
    ...
    // 서브 쿼리를 사용하여 나이를 조회하고 싶을때 ExperssionUtils.as를 사용하여 별칭을 사용하여 matching 시켜준다.
    @Test
